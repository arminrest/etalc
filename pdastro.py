...
class pdastroclass:
    ...
    def write(self,filename=None,indices=None,columns=None,formatters=None,
        ...
        #print('GGGGG0',formatters, self.default_formatters)
    ...
    def extract_regex(self,col,destcols,regex,indices=None):
        # get the indices based on input.
        indices=self.getindices(indices)  
        
        extract_results = self.t.loc[indices,col].str.extract(regex)
        if is_string_dtype(destcols):
            destcols=[destcols]
        if len(destcols)!=extract_results.shape[1]:
            raise RuntimeError('The number of destination cols is unequal the number of reg expression matches!')
        for i,destcol in enumerate(destcols):
            self.t.loc[indices,destcol] = extract_results[i].values
    ...
    def fitsheader2table(self,fitsfilecolname,indices=None,requiredfitskeys=None,optionalfitskey=None,nullval=np.nan,
